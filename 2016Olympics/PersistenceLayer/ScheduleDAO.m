...
-(int) remove:(Schedule*)model
{
    if ([self openDB]) {
		...
		if (sqlite3_prepare_v2(db, [sqlStr UTF8String], -1, &statement, NULL) == SQLITE_OK) {
            ...
            //绑定参数开始
			sqlite3_bind_int(statement, 1, (int)model.ScheduleID);
			//执行
			if (sqlite3_step(statement) != SQLITE_DONE) {
				NSAssert(NO, @"删除数据失败。");
			}
...
-(int) modify:(Schedule*)model
{
    ...
    if ([self openDB]) {
		...
		if (sqlite3_prepare_v2(db, [sqlStr UTF8String], -1, &statement, NULL) == SQLITE_OK) {
			...
			sqlite3_bind_int(statement, 2, (int)model.Event.EventID);
            ...
            sqlite3_bind_int(statement, 5, (int)model.ScheduleID);
            //执行
			if (sqlite3_step(statement) != SQLITE_DONE) {
				NSAssert(NO, @"修改数据失败。");
			}
